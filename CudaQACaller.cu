...
__device__ thrust::complex<float> * UbiPtr;
...
struct GreenCorner
{
	...
	thrust::complex <float> operator()(int idx) const 
	{
       ...
       if (idx >= 2*SIZE2_ + SIZE1_ && idx % (2*SIZE1_) >= SIZE1_) 
            return *(UbiPtr + q*SIZE2_ +  (SIZE1_ - 1) + ((id - 2*(SIZE2_ - SIZE1_) - 1) / (2*SIZE1_))*SIZE1_ - (2*SIZE1_ - 2 - (id - 1) % (2*SIZE1_)));
...

{
	...
	thrust::complex<float> operator()(thrust::complex<float> idxz) const
	{
		...
		if (abs (r.len()) > 0.0000001) return (*(UbiPtr + idx))* 
...
//cudaDeviceSynchronize ();
    thrust::device_vector <thrust::complex <float> > BigUi     (size3 * 4);
//cudaDeviceSynchronize ();
//cudaDeviceSynchronize ();
    thrust::device_vector <thrust::complex <float> > CurrentLayer (SIZE2_ * 4);
    thrust::device_vector <thrust::complex <float> > TempUi (SIZE2_ * 4);
//printf ("\n===size2*4 is %d===\n", SIZE2_*4);
//printf ("vectors done\n");
...
//printf ("\nffstarted\n");
    ...
    thrust::device_vector <thrust::complex <float> > Ui_plus_Ub    (size3);
    tempPtr = Ui_plus_Ub.data ().get ();
    	cudaMemcpyToSymbol(UbiPtr,
                     	  &tempPtr,
                     	  sizeof(void*));
    thrust::copy (Ui.begin(), Ui.end(), Ui_plus_Ub.begin());
    ...
    for (int j = 0; j < 10; j ++)
    {
        ...
        {  
            thrust::tabulate (TempUi.begin(), TempUi.end(), GreenCorner (i)); // returns Ui 
            cufftExecC2C(plan, reinterpret_cast<cufftComplex*> (TempUi.data ().get ()), 
                            reinterpret_cast<cufftComplex*> (TempUi.data ().get ()), CUFFT_FORWARD);      
            thrust::copy (TempUi.begin(), TempUi.end(), BigUi.begin() + i*4*SIZE2_); // returns Ui 
        }

        //Ub.assign(size3 , complex_t (0.0f, 0.0f));
        for (int l = 0; l < SIZE1_; l ++)
        {
            ...
            for (int q = 0; q < SIZE1_; q ++)
            {
                ...
                thrust::transform(BigBorn.begin(), BigBorn.end(),
                                  BigUi  .begin() + q*4*SIZE2_, BigBorn.begin(),
        ...
        thrust::transform (Ui.begin(), Ui.end(), Ub.begin(), Ui_plus_Ub.begin(), thrust::plus<thrust::complex<float>>());
    }
