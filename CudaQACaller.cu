...

{
	...
	thrust::complex<float> operator()(thrust::complex<float> idxz) const
	{
		...
		return *(UiPtr + idx) * inputDataPtr -> w2h3_ * (*(dSPtr + idx)) * thrust::exp(d_inputData -> uiCoeff_ * dr.len()) / (4 * _PI * dr.len());
	}
};
...

{
	...
	thrust::complex<float> operator()(thrust::complex<float> idxz) const
	{
		...
		return (*(UiPtr + idx)/(*(1 - *(UbPtr + idx) / *(UiPtr + idx)) * 
